...
import PostDetail from "./PostDetail"; // ✅ 추가
...
export default function Community() {
  ...
  const [selectedPost, setSelectedPost] = useState(null); // ✅ 클릭한 게시글 저장
  ...
  return (
    <div className="community-page">
        ...
        <div className="post-list">
          {filteredPosts.map((post) => (
            <div
              key={post.boardId}
              className="post"
              onClick={() => setSelectedPost(post)} // ✅ 게시글 클릭 시 상세보기 열림
            >
              <div className="post-header">
                <h3>{post.title}</h3>
                {currentUser?.email === post.user?.email && (
                  <div className="post-actions">
                    <button className="edit-btn" onClick={(e) => { e.stopPropagation(); openEditModal(post); }}>✏️</button>
                    <button className="delete-btn" onClick={(e) => { e.stopPropagation(); handleDelete(post.boardId, post.user?.email); }}>🗑️</button>
                  </div>
                )}
              </div>
      ...
      <div className="floating-container">
        <button className="floating-btn" onClick={() => setMenuOpen(!menuOpen)}>✏️</button>
        {menuOpen && (
